$(document).ready(function () {
    ...
    var indexs = url.split("/");
    var lastpart = indexs[indexs.length - 1];
    if (lastpart == "") {
        lastpart = "index";
    }
    getContent(lastpart, function(){
        ...
        $('.about-button').click(function(){
            window.location.href='subpages/about.php';
        });
    });


    function getContent(loc, callback) {
        var name = loc.split(".")[0];
        console.log(name);
        $.post("/mungerm/subpages/content.php", {name:name}, function(data){
            $('#main').html(data);
            callback()
        });
    }
